...
class Registro_Mascota_Activity : AppCompatActivity(),MascotaAdaptadorListener {
    ...
    fun LlenarTipoSp(){
        lifecycleScope.launch {
            ...
            var adaptador = object : ArrayAdapter<Tipos>(this@Registro_Mascota_Activity,android.R.layout.simple_spinner_dropdown_item,listaTipos){
    ...
    fun LlenarRazaSp(){
        lifecycleScope.launch {
            listRz=DB.ObtenerDB(this@Registro_Mascota_Activity).RazaDao().ObtenerRaza_Tipo(tipoSelect.id)
            var Adaptador= object : ArrayAdapter<Razas>(this@Registro_Mascota_Activity,android.R.layout.simple_spinner_item,listRz) {
                override fun getItemId(position: Int): Long {
    ...
    fun ObtenerMascota(){
        lifecycleScope.launch {
            listMascotas=DB.ObtenerDB(this@Registro_Mascota_Activity).MascotaDao().ObtenerTodoMascotas()
    ...
    fun ActualizarMascota(mascotas: Mascotas){
        lifecycleScope.launch {
            DB.ObtenerDB(this@Registro_Mascota_Activity).MascotaDao().ActualizarMascota(mascotas.id,mascotas.nombre_mascota,mascotas.fecha_nacimiento,mascotas.tipo_mascota_id,mascotas.raza_mascota_id)
            ObtenerMascota()
            LimpiarCampos()
        }
    }
    override fun onDeleteItemClick(mascotas: Mascotas) {
        lifecycleScope.launch {
            ...
            ObtenerMascota()
            Toast.makeText(this@Registro_Mascota_Activity, "Eliminado con exito", Toast.LENGTH_SHORT).show()
        }
    }
